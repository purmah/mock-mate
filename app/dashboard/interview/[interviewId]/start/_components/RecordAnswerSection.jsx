...
function RecordAnswerSection({ mockInterviewQuestion, activeQuestionIndex, interviewData }) {
    ...
    const {
        ...
        continuous: false,  // Ensure it does not listen continuously, will listen once when triggered
        useLegacyResults: false
    });

    useEffect(() => {
        // Whenever new results come in, append them to the userAnswer state
        results?.map((result) => (
    ...
    const StartStopRecording = async () => {
        if (isRecording) {
            stopSpeechToText();  // Stop the recording when it's active
        } else {
            setUserAnswer('');  // Clear previous answer on new start
